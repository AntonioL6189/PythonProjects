...
class HangmanGame:
    ...
    def initialize_gui(self):
        ...
        self.word_dispaly.pack(pady=(40, 20))
    ...
    def display_game_state(correct_guesses, incorrect_guesses,secret_word, attempts_left):
        #Display the secret word with guessed letters revealed
        displayed_word = " ".join([letter if letter in correct_guesses else "_" for
                                letter in secret_word])
        print(f"Word: {displayed_word}")
        print(f"Incorrect Guesses: {' '.join(incorrect_guesses)}")
        print(f"Attempts Left: {attempts_left}")   
    
    def check_game_over(correct_guesses, secret_word):
